"""
Gemini+Gemini+Murf FastAPI server — drop-in alternative to remote_server.py /
...
Exotel does not need URL changes; runs on port 7860 by default.
  ...
  python gemini_gemini_murf/server.py
...
# modules inside gemini_gemini_murf/ still resolve first.
...
async def websocket_endpoint(
    ...
    print("[GEMINI+GEMINI+MURF] WebSocket connection accepted")
    try:
        ...
        print(f"[GEMINI+GEMINI+MURF] Exception in run_bot: {e}")


@app.get("/fusion-mfi")
async def fusion_mfi_connect(request: Request) -> Dict[Any, Any]:
    print("[GEMINI+GEMINI+MURF] printing connect method args")
    ...
    print(f"[GEMINI+GEMINI+MURF] Generated WS URL for client: {ws_url}")
    return {"url": ws_url}


@app.get("/")
async def root_bot_connect(request: Request):
    print("[GEMINI+GEMINI+MURF] Handled root route request")
...
async def main():
    ...
    print(f"[GEMINI+GEMINI+MURF] Starting on port {port}")
