...
# ---------------------------------------------------------------------------
# Pricing — Gemini STT (USD) + Vertex Gemini LLM (USD) + Murf AI TTS (USD)
# ---------------------------------------------------------------------------

PRICING = {
    # Google STT (Chirp/Chirp 2/Chirp 3) — native USD per second
    "stt_google_per_sec_usd":  0.016 / 15.0,  # $0.064/min
...
def _compute_call_costs(
    *,
    ...
    # STT — Google STT (native USD).
    stt_usd = call_duration_s * PRICING["stt_google_per_sec_usd"]
...
def _build_google_stt(language: Language) -> GoogleSTTService:
    """Google Cloud Speech-to-Text streaming STT."""
    stt_model = os.getenv("GOOGLE_STT_MODEL", "chirp_2")
    stt_location = (
        os.getenv("GOOGLE_STT_LOCATION")
...
class CachingGoogleVertexLLMService(GoogleVertexLLMService):
    ...
    def __init__(
        ...
        *,
        ...
        cache_display_name: str = "gemini_agent_system_prompt",
...
async def run_voice_agent(websocket: WebSocket, config: AgentConfig) -> None:
    ...
    logger.info(
        f"Starting {config.name} call [GEMINI+GEMINI+MURF] | "
        f"lang={config.language} "
        f"stt={os.getenv('GOOGLE_STT_MODEL', 'chirp_2')} "
...
async def export_call_token_usage(
    ...
    """Append one row to token_logs/<date>/token_logs_<agent>_gemini_gemini_murf.csv."""
    try:
        ...
        log_file = os.path.join(log_dir, f"token_logs_{agent_name}_gemini_gemini_murf.csv")
