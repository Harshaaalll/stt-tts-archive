...
async def ws_endpoint(websocket: WebSocket, CustomField: str = None):
    ...
    try:
        ...
        print(f"[SIMPLE_SARVAM_AGENT] Exception: {e}")


@app.get("/fusion-mfi")
async def fusion_mfi_connect(request: Request) -> Dict[Any, Any]:
    print("[SIMPLE_SARVAM_AGENT] printing connect method args")
    ...
    print(f"[SIMPLE_SARVAM_AGENT] Generated WS URL for client: {ws_url}")
    return {"url": ws_url}


@app.get("/")
async def root(request: Request):
    print("[SIMPLE_SARVAM_AGENT] Handled root route request")
    return await fusion_mfi_connect(request)


async def main():
    port = int(os.environ.get("PORT", 7860))
    print(f"[SIMPLE_SARVAM_AGENT] Starting on port {port}")
