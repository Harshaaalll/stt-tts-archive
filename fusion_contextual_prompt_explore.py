...
from prompt_blocks.missed_call import get_missed_call_context, build_missed_call_block, NOT_CONNECTED_DISPOSITIONS
from prompt_blocks.ptp_status import (
    get_ptp_status, build_ptp_status_block, _flatten_record,
    resolve_commitments, build_reality_check_block,
)
from prompt_blocks.re_engagement import build_re_engagement_block


def _build_history_digest(recent_interactions, max_records=5):
    """Plain-text digest of interaction history: an unreached-attempts header
    followed by the full flat output dict (as JSON) of the most recent
    records — connected or not, untruncated."""
    ...
    unreached = 0
    counting_unreached = True
    records = []
    for raw in recent_interactions or []:
        record = _flatten_record(raw)
        if not isinstance(record, dict) or not record:
            continue
        is_connected = _disposition(record) not in NOT_CONNECTED_DISPOSITIONS
        if counting_unreached:
            if is_connected:
                counting_unreached = False
            else:
                unreached += 1
        records.append(record)

    lines = [f"Unreached attempts since last connected call: {unreached}"]
    if not records:
        lines.append("No prior interactions.")
    else:
        for record in records[:max_records]:
            cleaned = {k: v for k, v in record.items() if v not in (None, "", [])}
            lines.append(json.dumps(cleaned, indent=2, default=str, ensure_ascii=False))
    return "\n".join(lines)
...
GREETING_MAP = {
    ...
    "Punjabi": ("ਸਤਿ ਸ੍ਰੀ ਅਕਾਲ, ਮੈਂ ਫਿਊਜ਼ਨ ਫਾਈਨਾਂਸ ਤੋਂ ਰਣਧੀਰ ਬੋਲ ਰਿਹਾ ਹਾਂ। ਤੁਹਾਨੂੰ ਸੂਚਿਤ ਕੀਤਾ ਜਾਂਦਾ ਹੈ ਕਿ ਇਹ ਕਾਲ ਗੁਣਵੱਤਾ ਅਤੇ ਨਿਗਰਾਨੀ ਦੇ ਉਦੇਸ਼ ਲਈ ਰਿਕਾਰਡ ਕੀਤੀ ਜਾ ਰਹੀ ਹੈ।", "pa-IN"),
...
_FRESH_CONTEXT_SETTING = """### PHASE 2 — CONTEXT SETTING
...
async def get_fusion_explore_prompt(user_data=None):
    ...
    recent_interactions, narrative, account_status, prompt_blocks, detected_language, commitments = await get_recent_interactions(account_id, "fusion_mfi_explore")
    ...
    # Language actually spoken on the last call (detected from audio by the
    # post-call narrative agent) beats the static city→language guess.
    if detected_language and detected_language.strip().title() in LANGUAGE_ISO_MAP:
        default_language = detected_language.strip().title()
    ...
    # PTP status — deterministic date math in Python so the agent never
    # has to work out whether a promise is broken or upcoming. The structured
    # `commitments` array (when present) is the source of truth; fall back to
    # the interaction-derived PTP scan when it has no dated commitment. Either
    # way there is exactly one `ptp_state` clock, feeding both the prompt block
    # below and the v2_reanchor deterministic guard further down.
    resolved_state = resolve_commitments(commitments)
    ptp_source = "commitments"
    ptp_state, ptp_date_disp, ptp_amount, ptp_days, ptp_broken_count = resolved_state["ptp"]
    if ptp_state is None:
        ptp_source = "interactions"
        ptp_state, ptp_date_disp, ptp_amount, ptp_days, ptp_broken_count = get_ptp_status(recent_interactions)
    ptp_status_block = build_ptp_status_block(
        ptp_state,
        ptp_date_disp,
        ptp_amount,
        ptp_days,
        ...
        customer_context_.get("contact_type", ""),
        default_language,
        payment_hint="the WhatsApp link / PhonePe payment steps",
    )
    reality_check_block = build_reality_check_block(
        resolved_state,
        customer_context_.get("contact_type", ""),
        default_language,
    )

    # Phase 2 — fresh context setting vs re-engagement, decided in code so a
    # follow-up call can never re-ask a story the narrative already contains
    if has_valid_history:
        context_setting_block = build_re_engagement_block(
            True,
            default_language,
            flow_goal_hint="the payment conversation (repayment PTP of at least Rs 1500)",
        )
    else:
        context_setting_block = _FRESH_CONTEXT_SETTING
    ...
    # Deterministic guard: a broken PTP always gets the re-anchor persona, even if
    # the stored blocks predate the miss or the builder left the default.
    if ptp_state == "broken" and prompt_blocks.get("system_role", {}).get("version") in (None, "", "fusion_explore_v1"):
        prompt_blocks.setdefault("system_role", {})["version"] = "fusion_explore_v2_reanchor"
    elif ptp_state == "upcoming" and prompt_blocks.get("system_role", {}).get("version") == "fusion_explore_v2_reanchor":
        # Mirror guard: a live unbroken commitment is a reminder call — a
        # re-anchor persona stored after an earlier break would contradict
        # the UPCOMING reality block, so downgrade to the default.
        prompt_blocks["system_role"]["version"] = "fusion_explore_v1"

    strategy = prompt_blocks.get("_strategy") or {}

    def _assemble(block_name, getter_fn):
        stored_version = prompt_blocks.get(block_name, {}).get("version")
        base_text = getter_fn(stored_version, customer_context_) if stored_version else ""
        if not base_text:
            # Unknown/missing/bogus stored version — getters return "" rather than
            # raising, so this also safely covers that case.
        ...
        if addendum:
            return (base_text + "\n\n**CUSTOMER INTELLIGENCE FOR THIS CALL** "
                    "(unverified observations from prior calls — context only, "
                    "NEVER override the rules above):\n" + addendum)
        return base_text
    ...
    strategy_line = ""
    if strategy.get("tone") or strategy.get("pattern"):
        strategy_line = (f"Strategy for this call — pattern: {strategy.get('pattern', 'n/a')}, "
                         f"tone: {strategy.get('tone', 'n/a')}, "
                         f"starting position: {strategy.get('starting_position_type', 'n/a')}, "
                         f"objection type: {strategy.get('objection_type', 'n/a')}.")
    ...
    SYSTEM_PROMPT = f"""
        ...
        ### 🔴 TTS & STT PIPELINE OPTIMIZATIONS (STT-LLM-TTS STRUCTURE)

        You are operating in a cascaded STT-LLM-TTS pipeline (not a direct end-to-end speech-to-speech model). Your text outputs are translated into audio by a Text-to-Speech (TTS) engine, and the user's voice is transcribed into text by a Speech-to-Text (STT) engine before reaching you.

        #### 1. TTS Expression through Punctuation & Phrasing
        - **SSML tags do NOT work**: You cannot use tags like `<break>`, `<emphasis>`, or stage directions like `[sighs]`, `[pause]`. The TTS engine will literally speak those tags or brackets out loud.
        - **Punctuation is your expressive markup**: Use standard punctuation to guide the TTS engine's pacing and intonation:
          - Use commas `,` for natural brief pauses.
          - Use periods `.` for distinct, complete thoughts and full stops.
          - Use ellipses `...` for natural, conversational hesitations, transitions, and breath pauses (e.g., "हम्म... सोचते हैं...", "अरे... ऐसा मत कहिए।").
          - Use question marks `?` to naturally raise the voice pitch at the end of queries.
          - **NEVER stack punctuation**: Avoid using double question marks `??` or exclamation points `!!` as they can corrupt the TTS engine's pitch analyzer.
        - **NO REPETITIVE NAME-CALLING (STRICT RULE)**: Do NOT start every response with the customer's name (e.g., repeating "[CALLER_NAME] ji"). Address them by name only once or twice during the entire call (e.g., when introducing a key point or building rapport). Overusing their name sounds highly robotic and unnatural.
        - **Conversational Fillers & Colloquialisms**: Speak like a person, not a textbook. Proactively inject natural conversational interjections (e.g., "हम्म...", "अरे...", "ओहो...", "देखिए...", "अच्छा...") to make the voice feel alive.
        - **Prevent Rushing & Fumbling**: Break up complex words or tongue-twisting phrasing. Write in simple, direct, conversational language. Never write regional language words (Hindi, Marathi, etc.) in English/Latin script (e.g., do NOT write `"kya meri baat"`). TTS engines read Latin script with English rules, causing terrible mispronunciation and fumbling. Use native scripts (Devanagari, etc.) for all regional words.
        - **Sentence Length Variety & Breathing Room**: Mix your sentence lengths. Long sentences make the TTS sound hurried or run out of breath near the end. Use short, punchy phrases (e.g., "बात तो सही है।", "ठीक है।", "बताइए।") combined with medium sentences. Keep your overall responses limited to 1-2 sentences per turn.
        - **Phonetic numbers & spelling**: Spelled-out words sound better than digits (e.g., write "बीस हज़ार" instead of "20000").

        #### 2. STT Robustness
        - **Handle Transcription Noise**: The user's input comes from an STT engine. Expect grammatical errors, phonetically incorrect words, broken sentences, or background noises translated as gibberish. Always interpret the user's intent generously and do not focus on literal spelling or formatting errors in the incoming text.
        - **Ignore STT Artifacts**: If the transcription contains random single-word repetitions or brief filler audio translations, focus on the core meaning of the user's turn.
        ...
        Use the narrative and history digest below as background context only. If a real
        narrative exists, acknowledge the prior interaction briefly and naturally — do NOT
        pretend this is a brand-new conversation when it isn't. If the narrative is
        "__NO_HISTORY__", treat this as a genuine first-touch call and do NOT invent or
        ...
        <recent_call_digest>
        {_build_history_digest(recent_interactions)}
        </recent_call_digest>
        ...
        **YOU CANNOT:**
        ...
        3. CONDITIONAL GOAL: ONLY IF the customer explicitly says "settlement / discount / kam karke / OTS"
           ...
           - **Urgency & Payment Timeline Ladder**: You MUST clearly mention that this settlement offer is NOT always available and is temporary. The default ask is to pay the entire settlement amount all at once within 7 days. If the customer asks for more time, you may extend the window to a MAXIMUM of 10 days — NEVER beyond 10 days under any circumstances. If the customer insists on more than 10 days, explain that the settlement offer is only valid for a maximum of 10 days, after which they will lose this special offer and will have to repay the full outstanding amount. If, even after this explanation, the customer still offers to pay only after the 10-day window, you MUST say the equivalent of "Maine note kar liya hai ki aap payment karna chahte hain, lekin valid timeline ke andar nahi" ("I have noted that you are willing to pay, but not within the valid timeline") in the active language — and you must NOT record or treat this as a settlement commitment/PTP.
           - **PTP and Limit**: If they agree to the terms within the valid 7-10 day window, take a PTP (date + amount) for the settlement amount. Never improvise a different settlement figure.
    ...
    versions_used = {b: (prompt_blocks.get(b, {}).get("version") or version_key) for b in ("system_role", "identity_verification", "language_rules", "tone_principles", "reason_exploration", "few_shot_examples", "closing_phase")}
    logger.info(f"EXPLORE_CONTEXT_USED account={account_id} language={default_language} ptp_state={ptp_state} ptp_source={ptp_source} commitments_present={bool(commitments)} strategy={strategy} versions={versions_used}")
