"""
Sarvam+Groq+Murf FastAPI server — drop-in alternative to remote_server.py /
...
Exotel does not need URL changes; runs on port 7860 by default.
  ...
  python sarvam_groq_murf/server.py
...
# modules inside sarvam_groq_murf/ still resolve first.
...
async def websocket_endpoint(
    ...
    print("[SARVAM+GROQ+MURF] WebSocket connection accepted")
    try:
        ...
        print(f"[SARVAM+GROQ+MURF] Exception in run_bot: {e}")


@app.get("/fusion-mfi")
async def fusion_mfi_connect(request: Request) -> Dict[Any, Any]:
    print("[SARVAM+GROQ+MURF] printing connect method args")
    ...
    print(f"[SARVAM+GROQ+MURF] Generated WS URL for client: {ws_url}")
    return {"url": ws_url}


@app.get("/")
async def root_bot_connect(request: Request):
    print("[SARVAM+GROQ+MURF] Handled root route request")
...
async def main():
    ...
    print(f"[SARVAM+GROQ+MURF] Starting on port {port}")
