...
class LLMOutputSanitizer(FrameProcessor):
    ...
    _MAX_HOLD = 20  # longest recognized delimiter length
    ...
    def _consume(self, chunk: str, is_final: bool) -> str:
        ...
        while self._buf:
            ...
            if self._buf.startswith(self._SUPPRESS_OPEN):
                ...
                self._suppress = True
                continue
            # possible partial tag: hold until we see '>' or enough chars arrive
            if ">" not in self._buf and len(self._buf) < self._MAX_HOLD and not is_final:
                break
            # unknown/complete tag — pass '<' through literally
            out.append("<")
...
def _build_deepgram_stt(language: Language) -> DeepgramSTTService:
    ...
    try:
        utterance_end = int(os.getenv("DEEPGRAM_UTTERANCE_END_MS", "1000"))
    except ValueError:
        utterance_end = 1000
...
async def run_voice_agent(websocket: WebSocket, config: AgentConfig) -> None:
    ...
    def _strip_filler_prefix(content: str) -> str:
        ...
        # Try all known-injected phrases, and also each phrase without its
        # trailing suffix (Murf sometimes reformats "..." to ".. .").
        ...
        # Also try variants where "..." becomes ".. ."
