"""
Sarvam STT + Sarvam LLM (sarvam-m) + Murf AI TTS.
"""
...
_MAX_TOKENS = 200  # Short replies but leave headroom in case Sarvam's
                   # reasoning mode consumes some of the budget invisibly.
...
# ---------------------------------------------------------------------------
# Pricing — Sarvam STT (INR-native) + Sarvam LLM (INR-native) + Murf AI TTS (USD)
# ---------------------------------------------------------------------------
...
PRICING = {
    ...
    # Sarvam-M — approximate USD per 1M tokens (verify at billing).
    "llm_usd": {
        "sarvam-m":                {"input": 0.36,  "cache_read": 0.36,  "output": 1.20},
        "_default":                {"input": 0.36,  "cache_read": 0.36,  "output": 1.20},
    },
}
...
def _compute_call_costs(
    *,
    ...
    # Sarvam LLM — USD per 1M tokens (approx).
...
class LLMOutputSanitizer(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if name == "LLMTextFrame":
            ...
            cleaned = self._consume(raw, is_final=False)
            # Diagnostic: log raw vs cleaned only when they differ significantly
            if raw and not cleaned:
                logger.info(f"[sanitizer] raw={raw[:80]!r} -> DROPPED (suppress={self._suppress}, buf_len={len(self._buf)})")
            if cleaned:
                try:
                    frame.text = cleaned
                    await self.push_frame(frame, direction)
            ...
            return
        if name == "LLMFullResponseStartFrame":
            logger.info("[sanitizer] LLM response started")
        if name == "LLMFullResponseEndFrame":
            logger.info(f"[sanitizer] LLM response ended (suppress_state={self._suppress}, buf={self._buf[:60]!r})")
...
def _get_filler_config():
    # Sarvam LLM is fast enough (~450ms TTFB) that fillers add awkward
    # dead air rather than hiding it. Default OFF; enable via env for A/B.
    enabled = os.getenv("FILLER_ENABLED", "false").lower() in ("true", "1", "yes", "on")
    try:
...
_SARVAM_SHORT_REPLY_HINT = (
    "\n\n### 🔵 REPLY LENGTH — HARD CAP\n"
    "Every reply must be ONE short sentence (max ~20 words). If you have more "
    "to say, wait for the customer's next turn. Long replies during a phone "
    "call cause interruption cascades and break the conversation flow.\n"
)


def _build_sarvam_llm(system_instruction: str) -> SarvamLLMService:
    """Sarvam's Mumbai-hosted LLM (OpenAI-compatible API). No prompt
    caching — full system prompt is sent every turn."""
    model = os.getenv("SARVAM_MODEL", "sarvam-m")
    # Prepend a hard reply-length cap so Sarvam's output stays short enough
    # to avoid barge-in cascades during long TTS playback.
    sys_prompt = (system_instruction or "") + _SARVAM_SHORT_REPLY_HINT
    logger.info(
        f"[sarvam-llm] model={model} sys_prompt_chars={len(sys_prompt)} "
    ...
    return SarvamLLMService(
        ...
        settings=SarvamLLMService.Settings(
            ...
            reasoning_effort="low",  # keep visible-token budget intact
        ),
    )
...
def _build_tools_schema(extra_tools: Optional[ToolsSchema]) -> ToolsSchema:
    # `terminate_call` is intentionally omitted from Sarvam's tool schema —
    # Sarvam's tool-calling misfires on the long conditional description
    # (same failure mode as Groq). Call ends via Exotel hangup.
    extras = list(extra_tools.standard_tools) if extra_tools else []
    return ToolsSchema(standard_tools=extras)
...
async def run_voice_agent(websocket: WebSocket, config: AgentConfig) -> None:
    logger.info(
        f"Starting {config.name} call [SARVAM+SARVAM-LLM+MURF] | "
        f"lang={config.language} "
        f"stt={os.getenv('SARVAM_STT_MODEL', 'saarika:v2.5')} "
        f"llm={os.getenv('SARVAM_MODEL', 'sarvam-m')} "
    ...
    llm = _build_sarvam_llm(config.system_instruction)
    ...
    # Sarvam LLM's tool-calling has issues with the long `terminate_call`
    # description; skip registration and rely on Exotel-side hangup instead.
...
async def export_call_token_usage(
    ...
    """Append one row to token_logs/<date>/token_logs_<agent>_sarvam_sarvamllm_murf.csv."""
    try:
        ...
        llm_model = os.getenv("SARVAM_MODEL", "sarvam-m")
        ...
        log_file = os.path.join(log_dir, f"token_logs_{agent_name}_sarvam_sarvamllm_murf.csv")
