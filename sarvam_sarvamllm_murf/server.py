"""
Sarvam+SarvamLLM+Murf FastAPI server — drop-in alternative to remote_server.py /
...
Exotel does not need URL changes; runs on port 7860 by default.
  ...
  python sarvam_sarvamllm_murf/server.py
...
# modules inside sarvam_sarvamllm_murf/ still resolve first.
...
async def websocket_endpoint(
    ...
    print("[SARVAM+SARVAM-LLM+MURF] WebSocket connection accepted")
    try:
        ...
        print(f"[SARVAM+SARVAM-LLM+MURF] Exception in run_bot: {e}")


@app.get("/fusion-mfi")
async def fusion_mfi_connect(request: Request) -> Dict[Any, Any]:
    print("[SARVAM+SARVAM-LLM+MURF] printing connect method args")
    ...
    print(f"[SARVAM+SARVAM-LLM+MURF] Generated WS URL for client: {ws_url}")
    return {"url": ws_url}


@app.get("/")
async def root_bot_connect(request: Request):
    print("[SARVAM+SARVAM-LLM+MURF] Handled root route request")
...
async def main():
    ...
    print(f"[SARVAM+SARVAM-LLM+MURF] Starting on port {port}")
